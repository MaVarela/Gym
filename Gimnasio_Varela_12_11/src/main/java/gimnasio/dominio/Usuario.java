...
@NamedQueries({
	@NamedQuery(
	name = "getSociosNoInscriptos",
	query = "SELECT U FROM Usuario U WHERE U.rol.id = :idRol AND :idClase NOT IN (SELECT D.id.idDisciplina FROM U.disciplinas D)"
	),
	@NamedQuery(
	name = "getSociosInscriptos",
	query = "SELECT U FROM Usuario U WHERE U.rol.id = :idRol AND :idClase IN (SELECT D.id.idDisciplina FROM U.disciplinas D)"
	)
})
